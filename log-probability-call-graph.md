...
```mermaid
flowchart TD
    A["build_gp(x)"] --> A1["scenario.build_gp"]
    A1 --> A2["GaussianProcess.__init__"]

    A2 --> A3["vmap mean_function over X"]
    A2 --> A4["Diagonal noise from jitter^2"]
    A2 --> A5["QuasisepSolver.__init__"]

    A5 --> A6["kernel.to_symm_qsm(X)"]
    A5 --> A7["noise.to_qsm()"]
    A5 --> A8["matrix plus noise"]
    A8 --> A9["ops.elementwise_add"]
    A9 --> A10["deconstruct"]
    A9 --> A11["add_two"]
    A9 --> A12["construct"]
    A12 --> A13["SymmQSM matrix"]
    A13 --> A14["SymmQSM.cholesky()"]
    A14 --> A15["lax.scan Cholesky recursion"]
    ...
    B4 --> B8["p = h @ Pinf"]
    B8 --> B9["d = sum(p * q)"]
    B8 --> B10["vmap matrix multiply with a"]
    B10 --> B11["StrictLowerTriQSM"]
    B9 --> B12["DiagQSM"]
    B11 --> B13["SymmQSM"]

    B7 --> C1["Kernel-specific leaves"]
    C1 --> C2["Exp observation_model"]
    C1 --> C3["Exp transition_matrix"]
    C1 --> C4["Matern32 observation_model"]
    C1 --> C5["Matern32 transition_matrix"]
    C1 --> C6["Matern52 observation_model"]
    C1 --> C7["Matern52 transition_matrix"]
    ...
    D6 --> D7["lax.scan forward substitution"]
    D7 --> D8["alpha"]

    D2 --> D9["-0.5 * sum(alpha^2)"]
    D2 --> D10["solver.normalization()"]
    D10 --> D11["sum log factor.diag.d"]
    D10 --> D12["0.5 * N * log 2pi"]
    D9 --> D13["loglike"]
    D10 --> D13
    D13 --> D14["where finite else -inf"]
```
